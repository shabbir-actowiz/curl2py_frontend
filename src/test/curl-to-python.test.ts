...
describe("curl-to-python conversion", () => {
  it("converts a simple GET curl", () => {
    ...
    expect(python("curl 'https://example.com/api?q=milk'")).toContain("response = requests.get(url, impersonate=\"chrome\")");
  });

  it("converts POST JSON curl using json=payload", () => {
  ...
  it("keeps explicit POST with no body bodyless", () => {
    ...
    expect(code).toContain("response = requests.post(url, impersonate=\"chrome\")");
